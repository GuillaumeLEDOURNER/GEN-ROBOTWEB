...
/** A partir d'une URL de requête sur le site de référence et d'une expression exp, 
  ...
  def searchElement(html:Html) : List[(String,String)]
  
  def search(html: Html,results: List[(String,String)]):List[(String,String)]
}
 ...
 */
  def filtreAnnonce(h : Html): List[(String, String)]
